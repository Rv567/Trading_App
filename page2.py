...
def app():
    ...
    **Beta measures how much a stock's price moves compared to the overall market. It's calculated based on 3 Years data.**
    ...
    query = (Query()
            .select('name', 'close', "Change %","Net Income (Annual YoY Growth)", "Return on Equity (TTM)","current_ratio", "Price to Earnings Ratio (TTM)", 'Price to Book (FY)','dividends_yield',
                "Perf.YTD")
    ...
    df = df.rename(columns={"name":"Name","close":"Close","change":"Change %","price_earnings_ttm":"P/E","price_book_ratio":"P/B","dividends_yield":"Div Yield %","net_income_yoy_growth_fy":"Net Income Growth %","Perf.YTD":"Perf %","return_on_equity":"ROE %","current_ratio":"Current Ratio"})
